...
torch::Tensor matrixForward(const torch::Tensor image_tensor, const torch::Tensor thetas_tensor, const torch::Tensor positions_tensor) {
    ...
    torch::Tensor matrix_tensor = torch::zeros({sinogram_tensor.sizes()[0], 1, static_cast<signed long>(thetas_tensor.sizes()[0]*positions_tensor.sizes()[0]), static_cast<signed long>(image_tensor.sizes()[2]*image_tensor.sizes()[3])});
            ...
            for(int32_t position_idx = 0; position_idx < positions_tensor.sizes()[0]; position_idx++) {
                ...
                while(img_idx.x >= 0 && img_idx.x < image_tensor.sizes()[3] && img_idx.y >= 0 && img_idx.y < image_tensor.sizes()[2]) {
                    ...
                    if(fabsf(last_t_x+delta_t_x-last_t_y-delta_t_y) < FLOAT_CMP_THRESHOLD) {
                        ...
                        matrix[batch_idx][0][theta_idx*positions_tensor.sizes()[0]+position_idx][img_idx.y*image_tensor.sizes()[3]+img_idx.x] += last_t_x-t;
                        //Modify img_idx
                        switch(curr_case) {
                        ...
                        matrix[batch_idx][0][theta_idx*positions_tensor.sizes()[0]+position_idx][img_idx.y*image_tensor.sizes()[3]+img_idx.x] += last_t_y-t;
                        //Modify img_idx
                        switch(curr_case) {
    ...
    return matrix_tensor/(image_tensor.sizes()[3]*1.41421356237f);
}