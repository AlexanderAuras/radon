...
_cuda_impl      = typing.cast(typing.Any, torch.utils.cpp_extension.load(name="radon_impl",  sources=["./src/radon.cpp", "./src/forward.cu", "./src/backward.cu", "./src/filters.cu"]))
_radon_forward  = typing.cast(typing.Callable[[torch.Tensor,torch.Tensor,torch.Tensor],torch.Tensor], _cuda_impl.forward)
_radon_backward = typing.cast(typing.Callable[[torch.Tensor,torch.Tensor,torch.Tensor,int,int],torch.Tensor], _cuda_impl.backward)
del _cuda_impl
...
def radon_forward(image: torch.Tensor, angles: torch.Tensor = torch.linspace(0.0, 3.14159265359, 256, device="cuda"), positions: torch.Tensor|None = None) -> torch.Tensor:
    ...
    return _radon_forward(image, angles, positions)



def radon_backward(sinogram: torch.Tensor, image_size: int, angles: torch.Tensor = torch.linspace(0.0, 3.14159265359, 256, device="cuda"), positions: torch.Tensor|None = None, filter: int = 0) -> torch.Tensor:
    ...
    return _radon_backward(sinogram, angles, positions, image_size, filter)