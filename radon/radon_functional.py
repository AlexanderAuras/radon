...
def radon_forward(image: torch.Tensor, angles: torch.Tensor|None = None, positions: torch.Tensor|None = None) -> torch.Tensor:
    ...
    assert angles.is_contiguous(), "'angles' must be contiugous"
...
def radon_backward(sinogram: torch.Tensor, image_size: int, angles: torch.Tensor|None = None, positions: torch.Tensor|None = None) -> torch.Tensor:
    ...
    if sinogram.is_cuda:
        return _impl.cuda_backward(sinogram, angles, positions, image_size)
    return _impl.cpu_backward(sinogram, angles, positions, image_size)